...
```{r}
# BCRobs=read.csv("/data5/maolp/Gaofeng_CD8_matrix/Allcount//All_scanpyData/Data/BCRobs.csv")
# BCRobs
```
```{r}
...
process_TRB_data(TRB_data2,CDRlocus ) %>% 
  ...
  theme_classic()+
  ggtitle(paste0("CD8 TRB  ",CDRlocus))+
  theme_linedraw() +
  xlab('Length(nt)') +
  ylab('ratio(%)') +
...
```{r}
head(DCD8)
DCD82=DCD8[,c("AdjustedID","Alen","Freq_ratio")]

DCD82_wide <- DCD82 %>% 
  pivot_wider(names_from = Alen, values_from = Freq_ratio)

DCD82_wide <- spread(DCD82, Alen, Freq_ratio) %>% as.data.frame()
rownames(DCD82_wide)<-DCD82_wide$AdjustedID
DCD82_wide<-DCD82_wide[,-1]
```

```{r}
library(factoextra)
DCD82_pca <- prcomp(DCD82_wide)

fviz_pca_ind(DCD82_pca  , repel=T)
```

```{r}

groups=c(rep("Early PBMC",11),rep("Late PBMC",10))
P1=fviz_pca_ind(DCD82_pca,
...
PCD8=grid.arrange(P3V,PN1, PD,PN2,P5J, ncol = 1)

ggsave(plot=PCD8,"/home/maolp/Main_Gao_ScanpyProject20231130/HFB_Figure_Plot/Sup_Figure4/S24//CD8TRB.pdf", width = 6, height = 20, dpi = 300)
```
...
Immusig_plot(TRB_data2, c("N1-REGION", "N2-REGION", "3'V-REGION", "5'J-REGION", "D-REGION"), "TRB",
                         "/home/maolp/Main_Gao_ScanpyProject20231130/HFB_Figure_Plot/Sup_Figure4/S24sig/CD4TRBsig.pdf")
```
