...
```{r}
# load("/data1/maolp/Codeman/Project/Main_Gao_ScanpyProject20231003/Gao_plot/Figure3_TCRBCR_CDR3/DATA/CD8_MainTCR2_20231028.RData")
```
...
TCRobs
TCRobs<-subset(TCRobs,Last_cell_type=="Naïve CD8 T")
```
...
TRBp1 <- ggplot() +
  ...
  scale_fill_gradientn(colors = rev(c("#C71000FF", "#edae11", "white", '#246BAE')), 
                       na.value = "white") +
  theme_minimal() +
  labs(title = "PBMC CD8 TRB", x = "V gene", y = "J gene", fill = "Percentage(%)") +
...
print(TRBp1)
ggsave(plot = TRBp1,"/data5/maolp/Main_Gao_ScanpyProject20231130/HFB_Figure_Plot/Sup_Figure4/S25/S25A_TRB_CD8_VJcombined.pdf",width = 8,height = 4)
```
```{r}
...
TRAp1 <- ggplot() +
  ...
  scale_fill_gradientn(colors = rev(c("#C71000FF", "#edae11", "white", '#246BAE')), 
                       na.value = "white") +
  theme_minimal() +
  labs(title = "PBMC CD8 TRA", x = "V gene", y = "J gene", fill = "Percentage(%)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        plot.title = element_text(hjust = 0.5)) +
...
print(TRAp1)
  ggsave(plot = TRAp1,"/data5/maolp/Main_Gao_ScanpyProject20231130/HFB_Figure_Plot/Sup_Figure4/S25/S25A_TRA_CD8_VJcombined.pdf",width = 7,height = 7)
```
...
P3TRBplot=m1%>% mutate(Rcor=ifelse(cor<0,"R<0","R>0")) %>% mutate(VDJcol=factor(VDJcol,levels=c(subTRBsortgene,"Others TRB V-J"))) %>% 
  ...
  theme_classic() +
  labs(title = "PBMC CD8 TRB", x = "TRB V-J CDR3 max len", y = " Rcor",size="Percentage in TRB V-J(%)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
...
P3TRAplot=m1%>% mutate(Rcor=ifelse(cor<0,"R<0","R>0")) %>% mutate(VDJcol=factor(VDJcol,levels=c(subTRAsortgene,"Others TRA V-J"))) %>% 
  ...
  geom_hline(yintercept = 0, linetype="dashed", color = "black") +
  theme_classic() +
  labs(title = "PBMC CD8 TRA", x = "TRA V-J CDR3 max len", y = " Rcor",size="Percentage in TRA V-J(%)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        plot.title = element_text(hjust = 0.5),aspect.ratio = 0.6)+scale_y_continuous(limits = c(-1, 1),breaks = seq(-1,1,0.2))+scale_color_futurama()
P3TRAplot
```
...
```{r}
### 2023-10-28 23:30:58
save.image("DATA/CD8_VDJcombined.RData")
```
