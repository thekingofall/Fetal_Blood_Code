...
stat_compare_means(comparisons = my_comparisons, label = "p.signif", tip.length = 0.01) +
  facet_wrap(~variable, scales = "free") +theme(panel.grid = element_blank(), 
        ...
        legend.key = element_blank()) +scale_fill_futurama()+xlab("")+ylab("Percentage %")+scale_color_manual(values = pal_futurama()(3)[2:3])+ggtitle("TRAV CDR1")
PCDR1
```
```{r}
...
P1CDR1_len=df_perc2CDR1[grep("B", df_perc2CDR1$AdjustedID), ] %>%
  ...
  ggtitle("TRA CDR1")+theme(plot.title = element_text(hjust = 0.5))+scale_x_continuous(breaks = c(5,6))
# colnames()
P1CDR1_len
```
```{r}
ggsave(plot=P1CDR1_len,"Figure/TRA_P1CDR1_len.pdf")
```
...
PCDR1value=ggplot(PBMC_perc2CDR1_pvalue, aes(x = factor(variable), y = estimate, fill =pvalue_cat)) +
  ...
  theme_bw()+ggtitle("TRA CDR1(aa) lengths")+ 
  theme(plot.title = element_text(hjust = 0.5,  face = "bold"))+scale_y_continuous(limits = c(-0.6,0.6))
PCDR1value
ggsave(plot=PCDR1value,"Figure/TRA1_PCDR1value.pdf",width = 2,height = 2)
```
...
ggplot() + geom_line( aes(variable, value, color = Week,group=AdjustedID))+theme(panel.grid = element_blank(), 
                                                ...
                                                plot.title = element_text(hjust = 0.5),
                                                legend.title = element_blank(), 
                                                legend.key = element_blank())+ggtitle("TRA CDR2")+scale_x_continuous(breaks = c(5,6,7))+scale_color_gradientn(colors = rev(colorRampPalette(c("#FFA300","#6a73cf","#00AF99","#98D9E9" ))(100))) + labs(x="CDR2 length",y="Percentage(%)")->P2CDR2_len
P2CDR2_len
ggsave(plot=P2CDR2_len,"Figure/TRA_P2CDR2_len.pdf")
```
...
P3TRACDR2_value=ggplot(PBMC_perc2CDR2_pvalue, aes(x = factor(variable), y = estimate, fill =pvalue_cat)) +
    ...
    scale_fill_manual(values = colors) +
    labs(x = "Lengths", y = "R Value with Times", fill = "P Value") +
    theme_bw()+ggtitle("TRA CDR2(aa) lengths")+ 
...
PCDR2=ggplot(df_perc2CDR2[grep("B",df_perc2CDR2$AdjustedID),], aes(Stage, value)) + geom_boxplot(position = position_dodge(width = 1), show.legend = FALSE, color = "black")+geom_point(position = position_jitter(seed = 2023, width = .25),
  ...
  facet_wrap(~variable, scales = "free") +theme(panel.grid = element_blank(),
                                                ...
                                                plot.title = element_text(hjust = 0.5),
                                                legend.title = element_blank(),
                                                legend.key = element_blank()) +scale_fill_futurama()+scale_color_manual(values = pal_futurama()(3)[2:3])+xlab("")+ylab("Percentage %")+ggtitle("TRAV CDR2")
PCDR2
```
