...
```{r}
length(intersect(TCRobs$Cellname,subTRB$barcodename))
subTRBobs<-merge(subTRB,TCRobs,by.x="barcodename",by.y="Cellname")

```

```{r}
table(nchar(subTRBobs$cdr3))
```
```{r}
subTRBobs[,c("cdr3","IR_VDJ_1_junction_aa")]
subTRBobs2<-subTRBobs[!duplicated(subTRBobs$barcodename),]
# all(subTRBobs2$cdr3==subTRBobs2$IR_VDJ_1_junction_aa)
subTRBobs2<-subset(subTRBobs2,Last_cell_type=="Naïve CD4 T")
unique(subTRBobs2$Last_cell_type)
```
...
```{r}
subTRBobs2$CDR1length = nchar(subTRBobs2$cdr1)
table(subTRBobs2[, c("AdjustedID", "CDR1length")]) %>% as.matrix() %>% as.data.frame() -> dfCDR1
...
stat_compare_means(comparisons = my_comparisons, label = "p.signif", tip.length = 0.01) +
  facet_wrap(~variable, scales = "free") +theme(panel.grid = element_blank(), 
        ...
        legend.key = element_blank()) +scale_fill_futurama()+xlab("")+ylab("Percentage %")+scale_color_manual(values = pal_futurama()(3)[2:3])+ggtitle("CD4 TRBV CDR1")
PCDR1
```
```{r}
...
P1CDR1_len=df_perc2CDR1[grep("B", df_perc2CDR1$AdjustedID), ] %>%
  ...
  ggtitle("CD4 TRB CDR1")+theme(plot.title = element_text(hjust = 0.5))+scale_x_continuous(breaks = c(5,6,7))
# colnames()
P1CDR1_len
```
```{r}
ggsave(plot=P1CDR1_len,"Figure/CD4_TRB_P1CDR1_len.pdf")
```
...
PCDR1value=ggplot(PBMC_perc2CDR1_pvalue, aes(x = factor(variable), y = estimate, fill =pvalue_cat)) +
  ...
  theme_bw()+ggtitle("CD4 TRB CDR1(aa) lengths")+ 
  theme(plot.title = element_text(hjust = 0.5,  face = "bold"))+scale_y_continuous(limits = c(-1,1))
PCDR1value
ggsave(plot=PCDR1value,"Figure/CD4 TRB1_PCDR1value.pdf",width = 2,height = 2)
```

```{r}
subTRBobs2$CDR2length = nchar(subTRBobs2$cdr2)
table(subTRBobs2[, c("AdjustedID", "CDR2length")]) %>% as.matrix() %>% as.data.frame() -> dfCDR2
...
ggplot() + geom_line( aes(variable, value, color = Week,group=AdjustedID))+theme(panel.grid = element_blank(), 
                                                ...
                                                plot.title = element_text(hjust = 0.5),
                                                legend.title = element_blank(), 
                                                legend.key = element_blank())+ggtitle("CD4 TRB CDR2")+scale_x_continuous(breaks = c(5,6,7,8))+scale_color_gradientn(colors = rev(colorRampPalette(c("#FFA300","#6a73cf","#00AF99","#98D9E9" ))(100))) + labs(x="CDR2 length",y="Percentage(%)")->P2CDR2_len
P2CDR2_len
ggsave(plot=P2CDR2_len,"Figure/CD4_TRB_P2CDR2_len.pdf")
```
...
P3TRBCDR2_value=ggplot(PBMC_perc2CDR2_pvalue, aes(x = factor(variable), y = estimate, fill =pvalue_cat)) +
    geom_bar(stat = "identity") +
    scale_fill_manual(values = colors) +
    labs(x = "Lengths", y = "R Value with Times", fill = "P Value") +
    theme_bw()+ggtitle("CD4 TRB CDR2(aa) lengths")+ 
theme(plot.title = element_text(hjust = 0.5, face = "bold"))+scale_y_continuous(limits = c(-0.8,0.8),breaks = seq(-0.8,0.8,0.2))
P3TRBCDR2_value
```
```{r}
ggsave(plot=P3TRBCDR2_value,"Figure/TRB_P3TRBCDR2_value.pdf")
```
```{r}
df_perc2CDR2$variable <-
  as.numeric(as.character(df_perc2CDR2$variable))
...
PCDR2=ggplot(df_perc2CDR2[grep("B",df_perc2CDR2$AdjustedID),], aes(Stage, value)) + geom_boxplot(position = position_dodge(width = 1), show.legend = FALSE, color = "black")+geom_point(position = position_jitter(seed = 2023, width = .25),
  ...
  facet_wrap(~variable, scales = "free",ncol = 5) +theme(panel.grid = element_blank(),
                                                ...
                                                plot.title = element_text(hjust = 0.5),
                                                legend.title = element_blank(),
                                                legend.key = element_blank()) +scale_fill_futurama()+scale_color_manual(values = pal_futurama()(3)[2:3])+xlab("")+ylab("Percentage %")+ggtitle("CD4 TRBV CDR2")
PCDR2
```
