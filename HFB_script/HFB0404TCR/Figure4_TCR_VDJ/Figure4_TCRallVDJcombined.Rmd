...
### Load save Data
```{r}
# load("/data1/maolp/Codeman/Project/Main_Gao_ScanpyProject20231130/Gao_plot/Figure3_TCRBCR_CDR3/DATA/MainTCR2_20231026.RData")
```
...
#### Early_PBMC
```{r}
VDJheatmap(subVDJ,pattern =  "Early_PBMC",id_col="OrganStage",cluster_col=F,cluster_row=F,title = "Early_PBMC TRB")
```
#### Late_PBMC no cluster
```{r}
VDJheatmap(subVDJ, pattern = "Late_PBMC",id_col="OrganStage",cluster_col=F,cluster_row=F,title = "Late_PBMC TRB")
```

#### Late_PBMC cluster
```{r}
VDJheatmap(subVDJ, pattern = "Late_PBMC",id_col="OrganStage",cluster_col=T,cluster_row=T,title = "Late_PBMC TRB")
```
####  Early_PBMC cluster
```{r}
...
#### Early_Liver
```{r}
VDJheatmap(subVDJ,pattern = "Early_Liver",id_col="OrganStage",cluster_col=F,cluster_row=F,title = "Early_Liver TRB")
```

#### Early_Spleen
```{r}
VDJheatmap(subVDJ,pattern = "Early_Spleen",id_col="OrganStage",cluster_col=F,cluster_row=F,title = "Early_Spleen TRB")
```
#### Early_Thymus
```{r}
...
ggplot() +
  ...
  labs(x = "Week", y = "Percentage", color = "Top VDJ") +
  theme_classic()+scale_color_cosmic()
```
```{r}
library(ggsci)
top_VDJ=subset(PBMCTRB_cor_results,p.adjusted<0.1) %>% arrange(cor) %>% .$VDJ 
Sample_TRBVJ <- Sample_TRBVJ %>% 
  mutate(VDJ_Top10 = if_else(VDJ %in% top_VDJ, VDJ, "Other")) %>% 
mutate(VDJ_Top10=factor(VDJ_Top10,levels = c(as.character(top_VDJ),"Other")))
subset(Sample_TRBVJ,VDJ_Top10!= "Other") %>% 
...
```{r}
test1=VDJheatmap_data(subVDJ, pattern = "B",cluster_col=F,cluster_row=F,title = "PBMC")
```
```{r}
#   wide_data<-wide_data[,TRBinIMGT$IMGT_gene_name]
# setdiff(TRBIMGTsub$IMGT_gene_nam,colnames(test1))
```
```{r}
...
TRBp1 <- ggplot() +
  ...
  scale_fill_gradientn(colors = rev(c("#C71000FF", "#edae11", "white", '#246BAE')), 
                       na.value = "white") +
  theme_minimal() +
  labs(title = "PBMC All TRB", x = "V gene", y = "J gene", fill = "Percentage(%)") +
...
ggsave(plot = TRBp1,"/data5/maolp/Main_Gao_ScanpyProject20231130/HFB_Figure_Plot/Main_Figure4/Figure4E_TRB_all_VJcombined.pdf",width = 8,height = 4)
```
```{r}
...
TRAp1 <- ggplot() +
  ...
  scale_fill_gradientn(colors = rev(c("#C71000FF", "#edae11", "white", '#246BAE')), 
                       na.value = "white") +
  theme_minimal() +
  labs(title = "PBMC All TRA", x = "V gene", y = "J gene", fill = "Percentage(%)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        plot.title = element_text(hjust = 0.5)) +
...
print(TRAp1)
  ggsave(plot = TRAp1,"/data5/maolp/Main_Gao_ScanpyProject20231130/HFB_Figure_Plot/Main_Figure4/Figure4E_TRA_all_VJcombined.pdf",width = 7,height = 7)
```
...
P2=ggplot(subTRBvdjlen)+geom_tile(aes(Len,VDJ,fill=Percentage),color="white")+  scale_fill_gradientn(colors = rev(c('#C6403D', "#f6c619","white", '#246BAE')), 
                       na.value = "white") +
  theme_minimal() +
...
P3TRBplot=m1%>% mutate(Rcor=ifelse(cor<0,"R<0","R>0")) %>% mutate(VDJcol=factor(VDJcol,levels=c(subTRBsortgene,"Others TRB V-J"))) %>% 
  ...
  theme_classic() +
  labs(title = "PBMC TRB", x = "TRB V-J CDR3 max len", y = " Percentage in All(%)",size="Percentage in TRB V-J(%)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
...
P3TRAplot=m1%>% mutate(Rcor=ifelse(cor<0,"R<0","R>0")) %>% mutate(VDJcol=factor(VDJcol,levels=c(subTRAsortgene,"Others TRA V-J"))) %>% 
  ...
  geom_hline(yintercept = 0, linetype="dashed", color = "black") +
  theme_classic() +
  labs(title = "PBMC TRA", x = "TRA V-J CDR3 max len", y = " Percentage in All TRA V-J(%)",size="Percentage in TRA V-J(%)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        plot.title = element_text(hjust = 0.5),aspect.ratio = 0.6)+scale_y_continuous(limits = c(-1, 1),breaks = seq(-1,1,0.2))+scale_color_futurama()
P3TRAplot
```
...
```{r}
#2023-12-14 01:43:24
save.image("DATA/Figure4_TCRallVDJconbined.RData")
```
