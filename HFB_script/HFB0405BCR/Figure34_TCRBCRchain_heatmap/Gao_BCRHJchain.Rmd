...
```{r}
BCRHJ_chainlong=read.csv("/home/maolp/Allcount/All_scanpyData/BCRTCRchain/BCRHJ_chainlong.csv")
```
...
P2A=subset(BCRHJ2) %>% ggplot(aes(Organ_Stage,value,fill=Organ_Stage))+geom_boxplot(position = position_dodge(width = 1), outlier.size = 0.7, width = 0.1, show.legend = FALSE, color = "black")+ 
...
stat_compare_means(comparisons = my_comparisons, label = "p.signif", tip.length = 0) +
 ...
 P2A
 ggsave(plot = P2A,'Gao_TCRBCRchain_Figure/BCRHJ.pdf',width = 20,height = 40)
```
...
stat_compare_means(comparisons = my_comparisons, label = "p.signif", tip.length = 0) +
 ...
 P8A
 ggsave(plot = P8A,'Gao_TCRBCRchain_Figure/BCRHJ.pdf',width = 20,height = 40)
```
```{r}
my_comparisons <- list(
  c("Early_PBMC", "Late_PBMC")
